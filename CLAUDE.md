...
8. **BIS 오버레이 시즌 preview 툴팁** — `UI/BISOverlay.lua`는 Encounter Journal preview loot를 `itemID` 우선, `아이템명` fallback으로 찾는다. 구던전 BIS 아이템 툴팁 회귀가 나면 `getPreviewMythicPlusLootContext()`, `showSeasonItemTooltip()`, `openEncounterJournal()` 세 군데를 같이 봐야 한다.
9. **문장 / 열쇠 패널 통화 ID** — `UI/ItemLevelOverlay.lua`의 `CREST_ID_BY_GRADE`와 `DELVE_RESTORED_KEY_CURRENCY_ID`. `열쇠 파편`은 아직 안전한 itemID가 확정되지 않아 `-` fallback이 남아 있다.
10. **파티찾기 시즌 최고기록 아이콘 오버레이** — `UI/MythicPlusRecordOverlay.lua`는 이동형 프레임이 아니라 `ChallengesFrame.DungeonIcons` 위에 붙는 표시다. 위치/숨김 버그가 나면 `ChallengesFrame:Update()` 훅과 icon frame 재사용 경로를 먼저 확인한다.
...
- BIS 오버레이 아이템 행 클릭 → 모험 안내서 loot 탭 랜딩
- 드랍템 레벨 오버레이 우측 `나의 문장` / `나의 열쇠` 패널 표시 확인
- 파티찾기 시즌 최고기록 아이콘 오버레이의 `단수 / 평점 / 시간` 표시 확인
...
### BIS 인던 드랍 오버레이
`UI/BISOverlay.lua`. 전클래스/전특성 BIS 아이템을 부위별 섹션으로 표시한다. `Data/BISData_Method.lua`는 current overall BIS 기준 raid/crafted/mythicplus `sourceType/sourceLabel`을 갖고, `Data/BISData.lua` 수기 쐐기 데이터는 load 시 각 부위 `대체재 / 2순위`로 병합된다. 아이템 행 클릭 시 WoW 모험 안내서(EncounterJournal) loot 탭까지 랜딩을 시도한다. 툴팁은 시즌 1 preview hyperlink + 던전 트랙 요약을 함께 사용한다. 마우스 휠 스케일 지원.

### 아이템 레벨 오버레이 + 문장/열쇠 패널
`UI/ItemLevelOverlay.lua`. 던전/레이드/M+/제작 탭별 드랍 템렙 표 + 위대한 금고 컬럼. 쐐기/구렁/레이드/종합 표는 4열(`단/난이도`, `클리어보상`, `드랍문장`, `위대한 금고`)로 렌더되고, 우측 고정 패널에 `C_CurrencyInfo.GetCurrencyInfo(id)` 기반 현재 문장 보유량과 `오늘의 풍요 4개 / 열쇠 파편 / 복원된 열쇠`를 함께 표시한다. 구렁 탭에는 `풍요 열쇠 사용` 별도 행이 추가되어 있다.
- `CREST_ID_BY_GRADE` 테이블: 영웅(3345) 확인, 나머지 3341~3347 보정 사용
- `열쇠 파편`은 안전한 itemID 미확정으로 `-` fallback 가능

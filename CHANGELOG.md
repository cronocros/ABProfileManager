...
- 문서 구조 정리
  - 중복 사용자 문서와 유지 가치가 낮은 TODO/플레이스홀더 문서를 제거
  - 배포용 소개 자산 `ABProfileManager/ADDON_INTRO.txt`는 별도 목적 문서로 복구
  - 루트 README와 기술 문서를 목적 중심으로 다시 정리
  - 배포 패키지에 불필요하게 섞이던 애드온 내부 안내 문서를 제거

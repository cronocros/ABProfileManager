...
WoW 12.1.0 Midnight 시즌 2 대응. 아직 릴리스 전이며 아이템 레벨표의 일부 구간이
외부 자료 근거라 `scripts/run_season2_validation.ps1 -Strict`가 통과하지 않는다.
...
- BIS 데이터가 시즌 1 기준으로 동결된 상태를 `SeasonGuard`가 감지해 자동 랜딩과
  자동 점수화를 차단하고 기준 시즌을 표시

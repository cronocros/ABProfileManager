...
메모리 점검에서 `보통`으로 분류한 경로 15건입니다. 반복 계산과 재스캔을 캐시·스로틀로 줄였습니다. 자세한 목록은 `DOC/TODO.md` 3-1장에 있습니다.

`Data/BISRuntimeScoring.lua`의 음수 캐시 누락 1건은 동결 파일이라 손대지 않았습니다. `UI/BISOverlay.lua`의 툴팁 클로저 8개도 회귀 위험이 커서 남겼습니다.

## 2026-09-04 낮음 등급 8건

프레임 누수 2건, 중복 실행 2건, 기능 결함 2건, ruRU 치환 결함 1건, 지연 생성 1건입니다. 자세한 목록은 `DOC/TODO.md` 3-1장에 있습니다.

사용자에게 드러나던 것은 둘입니다. 러시아어에서 `Critical Strike`가 `치명ical Strike`처럼 잘못 치환되던 것과, 월드 이벤트 행을 좌클릭해도 완료 표시가 제자리로 돌아오던 것입니다. 후자는 해당 오버레이가 TOC에 없어 아직 로드되지 않습니다.

## 2026-09-04 M+ preview selector 확정

인게임 확인으로 시즌 2 Myth 1/6 selector가 `12849`임을 확정하고 반영했습니다. M+ 자동 점수화와 preview 툴팁이 다시 동작합니다. 레이드·티어·제작 selector는 아직 확인 전입니다.
...
- 전투 중과 창을 열어 둔 상태에서 애드온이 쓰던 자원을 줄였습니다. 지도와 BIS 목록, 전문기술 화면의 반복 계산도 줄였습니다.
- 러시아어에서 일부 능력치 이름이 잘못 번역되던 문제를 고쳤습니다.
- 쐐기 BIS 아이템의 실제 아이템 레벨과 스탯 미리보기가 다시 동작합니다.

...
이번 릴리스는 BIS 표시 규칙, Wowhead M+ fallback 보강, overlay 위치/크기 저장, hover 툴팁 복원, 구렁 표기 수정, idle CPU hot path 완화를 한 번에 정리한 유지보수 릴리스입니다.
...
- top BIS가 `mythicplus`가 아닌 슬롯만 Wowhead `Best Gear from Mythic+` 후보 기반 M+ fallback을 뒤에 붙이도록 병합 정책을 보강했습니다.
- BIS 아이템 hover는 기존 시즌 preview tooltip 경로를 다시 사용합니다.
- 제작 / 촉매는 계속 Encounter Journal 랜딩 대상에서 제외합니다.
- `scripts/refresh_wowhead_bis.py`와 `scripts/refresh_wowhead_mplus_fallbacks.py`로 BIS 두 데이터셋을 재생성할 수 있습니다.
...
- `StatsOverlay`는 raw state signature가 같으면 snapshot 재구성을 건너뛰고, `UNIT_AURA / UNIT_STATS / COMBAT_RATING_UPDATE` 계열을 느린 throttle로 분리해 idle 비용을 줄였습니다.
- `SilvermoonMapOverlay`는 상시 0.5초 polling 대신 월드맵 상호작용 시점의 짧은 burst refresh만 유지하고, 안정된 layoutKey가 확인되면 driver를 바로 내립니다.

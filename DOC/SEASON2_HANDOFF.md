...
| 야외 / 공격대 찾기 / `worldBoss` | 확정 (`279`, 노련가 1/6) | 인게임 툴팁 실측 |
| 영웅 던전 클리어 아이템 레벨 | 확정 (`276`, 모험가 4/6) | 인게임 툴팁 실측 |
| 영웅 던전 금고 아이템 레벨 | 미확정 | 인게임 금고 확인 |
...
### 시즌 2 던전 난이도별 아이템 레벨 (인게임 툴팁 실측)

같은 아이템(`교만의 나뭇가지`)을 난이도별로 확인했습니다. 출처는 `tooltip`입니다.
...
`모험가 4/6 = 276`이 확정된 랭크 사다리와 정확히 맞아 사다리를 다시 검증합니다. 신화 0 `292` 챔피언 1/6도 M+ 표와 일치합니다.

따라서 `mythicPlus.heroic`은 `ilvl 276`, `grade adv`, `maxilvl 282`입니다.

일반 던전은 강화 트랙이 없는 `214`입니다. 현재 `Data/ItemLevelTable.lua` 스키마에는 일반 던전 항목이 없으므로 이번 범위에서는 추가하지 않습니다.

영웅 던전의 위대한 금고 값은 아직 확인되지 않았습니다. 시즌 1 값 `243`(노련가)에 `+46`을 더하면 `289`이고 이는 확정된 사다리의 노련가 4/6과 같지만, 근거가 유도값이므로 인게임 확인 전까지는 `guide` 취급합니다.

### 야외 / 공격대 찾기 아이템 레벨 (인게임 툴팁 실측)

`해일을 꿰뚫는 자의 거품 폭발기` 툴팁에서 확인했습니다. 출처는 `tooltip`입니다.

- 아이템 레벨 `279`, 레벨 강화 `노련가 1/6`
- 야외 콘텐츠와 공격대 찾기가 같은 값입니다

와우헤드 레이드 표의 1보스 공격대 찾기 값 `279`(노련가 1/6)와 일치합니다.

이 값이 `worldBoss` 항목도 함께 해결합니다. 시즌 1 `worldBoss`는 `233`(노련가)였고 `+46`을 더하면 `279`로 실측값과 같습니다. 따라서 `worldBoss`는 `ilvl 279`, `grade vet`, `maxilvl 295`입니다.

Lair는 World부터 Mythic까지 난이도가 있으므로, World 난이도가 이 `279` 구간에 대응하고 상위 난이도는 레이드 표의 값(`292 / 305 / 318`)에 대응하는 구조로 보입니다. 즉 `worldBoss` 필드를 그대로 두고 값만 갱신하면 되며 스키마 변경이 필요 없습니다.

### 아직 없는 값

- PvP 명예 / 정복 아이템 레벨 (와우헤드에 시즌 2 페이지 없음, 인게임 상인 툴팁 필요)
- 영웅 던전의 위대한 금고 아이템 레벨 (클리어 값 `276`은 확정)
- (해결) `worldBoss`는 필드를 유지하고 값만 `279`로 갱신한다

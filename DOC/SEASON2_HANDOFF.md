...
| 2026-08-28 | 문장 툴팁 실측으로 트랙 범위 확정. `vet 282~295`, `chmp 295~308`, `hero 308~321`, `myth 321~334`. M+ 표와 3 차이 충돌 기록 |
...
| 항목 | 상태 | 확정 방법 |
| --- | --- | --- |
| 시즌 2 M+ 아이템 레벨 (신화 0 ~ +12) | 충돌 | 표 값과 문장 툴팁 트랙 시작값이 3 차이. 실제 드랍 툴팁 확인 필요 |
| 등급별 최대 아이템 레벨 (`gradeMax`) | vet/chmp/hero/myth 확정, adv 미확인 | 문장 툴팁 실측. 모험가 안개문장 툴팁만 남음 |
| 제작 base / r5 아이템 레벨 | 확정 (`318` / `331`) | 문장 툴팁 실측 |
| 구렁 최고 단계 | 확정 (`11` 유지) | 와우헤드 시즌 2 구렁 가이드 |
| 구렁 단계별 아이템 레벨 | 미확정 | 인게임 구렁 UI |
| 레이드 / PvP / Lair 아이템 레벨 | 미확정 | 인게임 툴팁 |
...
앞서 기록한 M+ 표는 `챔피언 1/6 = 292`, `영웅 1/6 = 305`, `신화 1/6 = 318`이라고 적고 있습니다. 그런데 문장 툴팁의 트랙 시작값은 `챔피언 295`, `영웅 308`, `신화 321`로 **각각 3만큼 높습니다.**

두 가지 가능성이 있습니다.

1. M+ 표가 인게임 화면이 아니라 커뮤니티 정리 자료였고 수치나 트랙 라벨이 어긋난 경우
2. M+ 드랍이 트랙 공식 시작값보다 낮은 지점에서 시작하고, 문장 툴팁은 문장으로 강화 가능한 구간만 표기한 경우

`ItemLevelTable`의 M+ 행은 `ilvl`과 `maxilvl`을 같이 담고 검증기가 `maxilvl == gradeMax[grade]`를 요구하므로, 이 충돌을 풀기 전에는 M+ 구간을 쓸 수 없습니다. 실제 M+ 드랍 아이템 툴팁 하나를 확인하는 것이 가장 확실합니다.

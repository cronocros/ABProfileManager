...
- 2026-09-03 DB2 대조로 확인한 것: 시즌 2 신화+ 던전 8종의 `JournalInstance` 한국어 이름 8건, 레이드 보스 9종의 `JournalEncounter` 한국어 이름, 티어 세트 13종의 `ItemSet` 한국어 이름이 모두 저장소 값과 글자 단위로 일치한다. 지도 별칭도 `UiMap` 한국어명과 맞는다. 다시 대조하지 않아도 된다.
- `SilvermoonMapData.runtimeInstances`의 `1312`는 확장팩 더미가 아니라 야외 우두머리 4종(바위소나무, 루아샬, 프레닥사스, 소름벨란)을 담은 `한밤` 인스턴스다. `1317`은 Lair `해일결속 동굴`(님리사 웨이브콜러), `1320`은 시즌 2 레이드 `맹독 심연`이다. 세 항목 모두 `raid` 분류가 맞다.
